...
{
    public class BASubscribe : ISubscribe
    {
        Int16 vCount = 0;
        DataTable dt = null;
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_DTLS_SUBSCRIBE_TOUR", CommandType.StoredProcedure))
                {
        ...
        public Byte SaveChanges<T1, T2>(String pMode, EntitySubscribe oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount] = new SqlParameter("@SUBSCRIBE_TOUR_KEY", SqlDbType.Int);
                para[vCount++].Value = oEntity.SUBSCRIBE_TOUR_KEY;
                para[vCount] = new SqlParameter("@EMAIL_ID", SqlDbType.VarChar, 50);
                para[vCount++].Value = oEntity.EMAIL_ID;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_DTLS_SUBSCRIBE_TOUR", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@MAST_CITY_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
