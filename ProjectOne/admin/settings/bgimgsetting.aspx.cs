...
namespace ProjectOne.admin.settings
{
    public partial class bgimgsetting : System.Web.UI.Page
    {
        ...
        private String ProcessForum(ref String Doc_Name, FileUpload fu_Control)
        {
            String[] ImageAcceptedExtensions = null;
            String DOC_PATH = "~" + ConfigurationManager.AppSettings["BACKGROUND"].ToString();
            try
            {
                if (fu_Control.HasFile)
                {
                    ...
                    {
                        HttpFileCollection attachments = Request.Files;
                        HttpPostedFile attachment = attachments[14];
                        if (attachment.ContentLength < 20097134)
                        {
        ...
        protected void btn_Head_Save_Click(object sender, EventArgs e)
        {
            ...
            AVAILABILITY_IMG = String.Empty, CALENDER_IMG = String.Empty, VOTING_IMG = String.Empty, FORUM_IMG = String.Empty; ;
            ...
            try
            {
                if (Page.IsValid)
                {
                                                            ...
                                                            if (String.IsNullOrEmpty(errMsg))
                                                            {
                                                                ...
                                                                if (String.IsNullOrEmpty(errMsg))
                                                                {
                                                                    ...
                                                                    if (String.IsNullOrEmpty(errMsg))
                                                                    {
                                                                        ...
                                                                        if (String.IsNullOrEmpty(errMsg))
                                                                        {
                                                                            errMsg = ProcessForum(ref FORUM_IMG, fu_Forum);
                                                                            if (!String.IsNullOrEmpty(errMsg))
                                                                            {
                                                                                MessageBox(1, errMsg, "");
                                                                                return;
                    ...
                    oBMAST.BG_FORUM_IMAGE = hf_FORUM_IMG.Value;
                    ...
                    {
                        vRef = oBMC.SaveChanges<Object, Int32>("", oBMAST, null, ref vKey, ref errMsg, "2019", 1);
                        if (vRef == 1)
                        {
                            MessageBox(1, "Data stored successfully", "");
                        }
        ...
        private String FillBackgroundImageSettingEdit()
        {
            try
            {
                ...
                errMsg = String.Empty;
                EntityHeadHomeBgSetting oBMST = new EntityHeadHomeBgSetting();
                dt = new DataTable();
                using (BAHeadHomeBgSetting oBME = new BAHeadHomeBgSetting())
                {
                    dt = oBME.Get("GET", 1, "", ref errMsg, null, 1);

                    oBMST.BG_ABOUT_ME_IMAGE = dt.Rows[0]["BG_ABOUT_ME_IMAGE"].ToString();
                    oBMST.BG_TOUR_IMAGE = dt.Rows[0]["BG_TOUR_IMAGE"].ToString();
                    oBMST.BG_QUICK_FACTS_IMAGE = dt.Rows[0]["BG_QUICK_FACTS_IMAGE"].ToString();
                    oBMST.BG_PREFER_IMAGE = dt.Rows[0]["BG_PREFER_IMAGE"].ToString();
                    oBMST.BG_GALLERY_IMAGE = dt.Rows[0]["BG_GALLERY_IMAGE"].ToString();

                    oBMST.BG_EXCITING_OFFER_IMAGE = dt.Rows[0]["BG_EXCITING_OFFER_IMAGE"].ToString();
                    oBMST.BG_DAIRY_IMAGE = dt.Rows[0]["BG_DAIRY_IMAGE"].ToString();
                    oBMST.BG_CLIENT_IMAGE = dt.Rows[0]["BG_CLIENT_IMAGE"].ToString();
                    oBMST.BG_SELFIE_IMAGE = dt.Rows[0]["BG_SELFIE_IMAGE"].ToString();
                    oBMST.BG_NEWS_IMAGE = dt.Rows[0]["BG_NEWS_IMAGE"].ToString();

                    oBMST.BG_RATE_IMAGE = dt.Rows[0]["BG_RATE_IMAGE"].ToString();
                    oBMST.BG_AVAILABILITY_IMAGE = dt.Rows[0]["BG_AVAILABILITY_IMAGE"].ToString();
                    oBMST.BG_CALENDER_IMAGE = dt.Rows[0]["BG_CALENDER_IMAGE"].ToString();
                    oBMST.BG_VOTING_IMAGE = dt.Rows[0]["BG_VOTING_IMAGE"].ToString();
                    oBMST.BG_FORUM_IMAGE = dt.Rows[0]["BG_FORUM_IMAGE"].ToString();
                }

                if (oBMST.BG_ABOUT_ME_IMAGE == "")
                {
                    img_About.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                    img_About.Src = "~" + ConfigurationManager.AppSettings["BACKGROUND"].ToString() + Convert.ToString(dt.Rows[0]["BG_ABOUT_ME_IMAGE"]);
                }
                if (oBMST.BG_TOUR_IMAGE == "")
                {
                    img_Tour.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                ...
                if (oBMST.BG_QUICK_FACTS_IMAGE == "")
                {
                    img_Quick_Facts.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                ...
                if (oBMST.BG_PREFER_IMAGE == "")
                {
                    img_Prefer.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_GALLERY_IMAGE == "")
                {
                    img_Gallery.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_EXCITING_OFFER_IMAGE == "")
                {
                    img_Offer.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_DAIRY_IMAGE == "")
                {
                    img_Diary.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_CLIENT_IMAGE == "")
                {
                    img_Testimonials.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_SELFIE_IMAGE == "")
                {
                    img_Selfie.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_NEWS_IMAGE == "")
                {
                    img_News.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_RATE_IMAGE == "")
                {
                    img_Rates.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                    img_Rates.Src = "~" + ConfigurationManager.AppSettings["BACKGROUND"].ToString() + Convert.ToString(dt.Rows[0]["BG_RATE_IMAGE"]);
                }
                if (oBMST.BG_AVAILABILITY_IMAGE == "")
                {
                    img_Availability.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_CALENDER_IMAGE == "")
                {
                    img_Calender.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_VOTING_IMAGE == "")
                {
                    img_Voting.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                ...
                if (oBMST.BG_FORUM_IMAGE == "")
                {
                    img_Forum.Src = ResolveClientUrl("../assets/images/no_image.jpg");
                }
                else
                {
                    img_Forum.Src = "~" + ConfigurationManager.AppSettings["BACKGROUND"].ToString() + Convert.ToString(dt.Rows[0]["BG_FORUM_IMAGE"]);
                }
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    hf_FORUM_IMG.Value = Convert.ToString(dt.Rows[0]["BG_FORUM_IMAGE"]);
                }
                return errMsg;
            }
            catch (Exception ex)
